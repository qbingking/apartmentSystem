...
<div class="col-12 p-0">
    <ul class="nav nav-tabs tabs-bordered nav-justified">
        ...
        <!-- <li class="nav-item">
            <a href="#apm_images<? // $item_apm['id'] ?>" data-toggle="tab" aria-expanded="false" class="nav-link">
                <i class="dripicons-camera mr-2"></i>
            </a>
        </li> -->
        <!-- <li class="nav-item">
            <a href="#apm_map<? // $item_apm['id'] ?>" data-toggle="tab" aria-expanded="false" class="nav-link">
                <i class="dripicons-location mr-2"></i>
            </a>
        </li> -->
    </ul>
    <div class="tab-content" >
        <div class="tab-pane" id="apm_service_<?= $item_apm['id'] ?>">
            ...
            ?>
            <?php $this->load->view('components/apm-services-mobile', $data); ?>
        </div>
        <div class="tab-pane active" id="apm_note_<?= $item_apm['id'] ?>">
            ...
            <blockquote class="blockquote blockquote-custom bg-white p-4 shadow rounded">
                <div class="blockquote-custom-icon bg-info shadow-sm copy-text" data-clipboard-target="#note-<?= $item_apm['id']?>" ><i class="fa fa-quote-left text-white btn"></i></div>

                <span class="mb-0 mt-1 font-italic note <?= in_array($district_key, $list_district_editable) != 0 ? 'note-editable':''  ?>">
        ...
        <div class="tab-pane" id="apm_room_<?= $item_apm['id'] ?>">
            ...
            /*Get all room by ROOM.id_apartment = id of apartment */
                ...
                $th_room = in_array($district_key, $list_district_editable) != 0 ? "<th></th>":'';
                 
             ?>
            <div class="table-responsive">
                
                <table id="room_datatable_<?= $item_apm['id'] ?>" class="datatable apm_room table table-bordered">
                    <div class="addRow mb-1 <?= (in_array($district_key, $list_district_editable) != 0) && ($editable == 1) ? '':'d-none' ?>" id="btn-<?= $item_apm['id'] ?>"><button class="btn btn-success">+</button></div>
                    <thead>
                        <tr class="text-muted">
                            <th>Mã</th>
                    ...
                    <tbody>
                        ...
                        <?php foreach ($list_room as $key_room => $item_room): ?>
                            <tr>
                                <td id="maphong-<?= $item_room['id'] ?>" <?= $editableRoom ?>><?= $item_room['maphong'] ?></td>
                                <td id="id_type-<?= $item_room['id'] ?>" <?= $editableRoom ?>><?= $item_room['id_type'] ?></td>
                                <td id="price-<?= $item_room['id'] ?>" <?= $editableRoom ?> class="font-weight-bold"><?= $item_room['price'] ?></td>
                                <td id="square-<?= $item_room['id'] ?>" <?= $editableRoom ?>><?= $item_room['square'] ?></td>
                                <td id="id_status-<?= $item_room['id'] ?>"
                                    class="<?= $editableRoomStatus ?>">
                                    <?= $item_room['id_status'] ?>
                                </td>
                                <td id="saptrong-<?= $item_room['id'] ?>" <?= $editableRoom ?>><?= $item_room['saptrong'] ?></td>
                                <td>
                                    <?php 
                                    $td_room = '
                                     ...
                                     ';
                                    echo (in_array($district_key, $list_district_editable) != 0 AND $editable) ? $td_room:"";
                                 ?>
                                </td>
        ...
        <div class="row container float-right">
           ...
           class="col-2 p-0 justify-content-center " 
